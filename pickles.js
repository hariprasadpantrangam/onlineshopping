...
// 8. అడ్మిన్ CRUD ఆపరేషన్లు (Rectified Edit/Update Logic)

let editIndex = null; // This tracks if we are editing an existing item

function editProduct(index) {
    editIndex = index; // Set the index to the item we clicked
    ...
    // Change the button text to show we are in "Update" mode
    // (Optional: requires an ID on your 'Add Product' button)
    const btn = document.querySelector("#adminPanel button[onclick='saveProduct()']");
...
function saveProduct() {
    ...
    const updatedProduct = { name, price, image, status: status || "Available" };

    if (editIndex !== null) {
        // UPDATE: Replace the item at the specific index
        products[editIndex] = updatedProduct;
        editIndex = null; // Reset back to "Add" mode
        alert("Product Updated Successfully!");
        
        // Reset button text if changed
        const btn = document.querySelector("#adminPanel button[onclick='saveProduct()']");
        if(btn) btn.innerText = "Add Product";
    } else {
        // ADD: Push new item to the array
